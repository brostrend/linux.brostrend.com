...
For Pi4, Pi400 and CM4, it's possible to undo this change by adding
`arm_64bit=0` in the `/boot/firmware/config.txt` file (or for older Raspberry
Pi OS versions, in `/boot/config.txt`). You can do that with any editor, or
...
For Pi5, Pi500 or CM5, it's not possible to use the aforementioned workaround,
as they can't boot with 32bit kernels.
