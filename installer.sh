...
log() {
    logger -t rtl8812au-dkms -p syslog.err "$@"
}
...
main() {
    ...
    cd $(mktemp -d)
    wget https://deb.trendtechcn.com/rtl8812au-dkms.deb || die "Couldn't download the driver"
    ...
    if dpkg --compare-versions $(dpkg-query -W apt | awk '{ print $2 }') gt 1.2; then
        ...
        apt install --yes --no-install-recommends ./rtl8812au-dkms.deb
    else
        ...
        dpkg -i ./rtl8812au-dkms.deb
    fi

    ret=$?
    printf "\n"
    if [ $ret -eq 0 ]; then
        bold "The driver was successfully installed."
    else
        bold "The driver was NOT successfully installed."
    fi
    pause_exit $ret
}
